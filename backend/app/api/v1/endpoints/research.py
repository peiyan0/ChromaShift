...
def _get_analytics_dict(db, participant_ids=None):
    ...
    for s in all_surveys:
        try:
            ...
            # Check for default/empty dummy responses
            is_default = False
            if all(v == 3 for v in q_odd + q_even):
                nasa_vals = [s.nasa_mental, s.nasa_physical, s.nasa_temporal, s.nasa_performance, s.nasa_effort, s.nasa_frustration]
                if all(v == 10 for v in nasa_vals):
                    comfort_vals = [s.comfort_q1, s.comfort_q2, s.comfort_q3, s.comfort_q4, s.comfort_q5]
                    if all(v == 3 for v in comfort_vals):
                        interviews = [s.interview_visual_transitions, s.interview_naturalness, s.interview_wizard_onboarding, s.interview_frustrating_aspects, s.interview_helpful_aspects]
                        
                        open_fb = s.interview_open_feedback or ""
                        # Remove the template headers and whitespace to see if they actually wrote anything
                        cleaned_fb = open_fb.replace("Daily Life Integration:", "").replace("Daily Useful Scenarios:", "").replace("General Feedback/Comments:", "").strip()
                        
                        if all(not val or val.strip() == "" for val in interviews) and cleaned_fb == "":
                            is_default = True
